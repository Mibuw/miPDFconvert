	...
	PDWORD pcbOutputNeeded)
{
	...
	{
		static WCHAR szUIDLL[] = L"miMonitorUI.dll";
...
LPMONITOR2 WINAPI InitializePrintMonitor2(PMONITORINIT pMonitorInit, PHANDLE phMonitor)
{
	...
	g_pLog->Log(LOGLEVEL_ALL, L"miMonitor is running");
	...
	g_pLog->Log(LOGLEVEL_ALL, L"InitializePrintMonitor2 successfully initialized miMonitor");
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD dwReason, LPVOID lpvReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		g_pLog = new CMfmLog();
		g_pLog->Log(LOGLEVEL_NONE, L"*** miMonitor log start ***");
	...
	case DLL_PROCESS_DETACH:
		delete g_pPortList;
		g_pLog->Log(LOGLEVEL_NONE, L"*** miMonitor log end ***");
		delete g_pLog;
		break;
	}
